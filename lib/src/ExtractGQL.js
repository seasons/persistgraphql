...
var ExtractGQL = (function () {
    ...
    ExtractGQL.prototype.createMapFromDocument = function (document) {
        ...
        queryDefinitions.forEach(function (transformedDefinition) {
            ...
            var definition = transformedQueryWithFragments
                .definitions[0];
            var opName = definition.name.value;
            result[opName] = docQueryKey;
        });
        return result;
    };
    ...
    ExtractGQL.prototype.getQueryFragments = function (document, queryDefinition) {
        ...
        var reduceQueryDefinitions = function (carry, definition) {
            ...
            if (extractFromAST_1.isFragmentDefinition(definition) &&
                queryFragmentNames[definitionName.value] === 1) {
                var definitionExists = carry.findIndex(function (value) {
                    return value.name.value === definitionName.value;
                }) !== -1;
                if (!definitionExists) {
        ...
        retDocument.definitions = document.definitions
            .reduce(reduceQueryDefinitions, [])
            .sort(common_1.sortFragmentsByName);
        return retDocument;
    };
    ...
    ExtractGQL.prototype.writeOutputMap = function (outputMap, outputFilePath) {
        return new Promise(function (resolve, reject) {
            fs.open(outputFilePath, "w+", function (openErr, fd) {
                ...
                fs.write(fd, JSON.stringify(outputMap, null, 2), function (writeErr, written, str) {
                    if (writeErr) {
                        reject(writeErr);
                    }
                    resolve();
    ...
    ExtractGQL.prototype.extract = function () {
        ...
        this.processInputPath(this.inputFilePath)
            .then(function (outputMap) {
            _this.writeOutputMap(outputMap, _this.outputFilePath)
                .then(function () {
